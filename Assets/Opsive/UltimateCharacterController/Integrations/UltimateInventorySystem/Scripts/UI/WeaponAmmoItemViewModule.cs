    ...
    public class WeaponAmmoItemViewModule : ItemViewModule
    {
        ...
        protected virtual void DrawAmmo(ItemInfo info)
        {
            ...
            for (int i = 0; i < characterItem.ItemActions.Length; ++i) {
                var shootableWeapon = characterItem.ItemActions[i] as ShootableWeapon;
                if (shootableWeapon == null) { continue; }

                var consumableItemIdentifierAmount = shootableWeapon.GetConsumableItemIdentifierAmount();
                ...
                if (m_AmmoItemImage != null) {
                    var ammoItem = (shootableWeapon.GetConsumableItemIdentifier() as Item);
                ...
                m_LoadedCount.text = shootableWeapon.GetConsumableItemIdentifierAmount().ToString();
                m_ClipSize.text = shootableWeapon.ClipSize.ToString();
                m_UnloadedCount.text = characterItem.Character.GetCachedComponent<CharacterInventoryBridge>().GetItemIdentifierAmount(shootableWeapon.GetConsumableItemIdentifier()).ToString();
                break;
            }
        ...
        private void OnAdjustItemIdentifierAmount(IItemIdentifier item, int consumableAmount)
        {
            DrawAmmo(ItemInfo);
        }

        private void OnUseConsumableItemIdentifier(Items.Item characterItem, IItemIdentifier item, int consumableAmount)
        {
            DrawAmmo(ItemInfo);
        }
        ...
        private void UnregisterToItemAttributeChangeEvent()
        {
            ...
            if (m_CachedInventory != null && m_CachedInventory.gameObject != null) {
                Shared.Events.EventHandler.UnregisterEvent<Items.Item, IItemIdentifier, int>(m_CachedInventory.gameObject, "OnItemUseConsumableItemIdentifier", OnUseConsumableItemIdentifier);
                Shared.Events.EventHandler.UnregisterEvent<IItemIdentifier, int>(m_CachedInventory.gameObject, "OnInventoryAdjustItemIdentifierAmount", OnAdjustItemIdentifierAmount);
            }
