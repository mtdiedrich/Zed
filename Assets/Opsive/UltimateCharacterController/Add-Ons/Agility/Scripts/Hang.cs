...
#if ULTIMATE_CHARACTER_CONTROLLER_CLIMBING
            StartFromHorizontalClimb,   // The character can start hanging from a horizontal climb position.
            StartFromVerticalClimb,     // The character can start hanging from a vertical climb position.
#endif
        }
        ...
        public override void Update()
        {
            ...
            if (m_HangState == HangState.Shimmy) {
                ...
                if (m_DrawDebugLines && Mathf.Abs(m_CharacterLocomotion.RawInputVector.x) > m_ChangeStateSensitivity) {
                    Debug.DrawRay(m_RaycastResult.point + MathUtility.TransformDirection(Vector3.right * m_ShimmyOffset * (m_CharacterLocomotion.RawInputVector.x < 0 ? -1 : 1),
                ...
                if (Mathf.Abs(m_CharacterLocomotion.RawInputVector.x) > m_ChangeStateSensitivity &&
                        m_CharacterLocomotion.SingleCast(m_Transform.right * m_ShimmyOffset * (m_CharacterLocomotion.RawInputVector.x < 0 ? -1 : 1),
