    ...
    public class LadderClimb : TraversalClimb
    {
        ...
        public override bool CanStartAbility()
        {
            ...
            if (m_CharacterLocomotion.Grounded) {
                var position = m_Transform.TransformPoint(0, -0.5f, m_CastDistance);
        ...
        public override MoveTowardsLocation[] GetMoveTowardsLocations()
        {
            ...
            offset.y = m_MoveTowardsLocation[0].transform.InverseTransformPoint(m_Transform.position).y;
        ...
        public override void Update()
        {
            ...
            if (inputVector.y > 0) {
                var localDismountLocation = m_Transform.InverseTransformPoint(m_Ladder.TopDismountLocation);
                ...
                var localDismountLocation = m_Transform.InverseTransformPoint(m_Ladder.BottomDismountLocation);
                if (localDismountLocation.y >= 0) {
        ...
        public override void UpdateRotation()
        {
            ...
            var targetRotation = Quaternion.LookRotation(m_Ladder.GetLookDirection(m_Transform), m_Ladder.transform.up);
            var deltaRotation = (Quaternion.Inverse(m_Transform.rotation * Quaternion.Euler(m_CharacterLocomotion.DeltaRotation)) * targetRotation);
        ...
        public override void ApplyPosition()
        {
            ...
            if (m_ClimbState == ClimbState.AirMount){
                ...
                var targetPosition = Vector3.MoveTowards(transformPosition, m_AirborneMountPosition, m_AirborneMoveTowardsSpeed * m_CharacterLocomotion.TimeScale);
                m_CharacterLocomotion.MoveDirection = targetPosition - transformPosition;
                // The character has arrived on a rung.
                if (m_CharacterLocomotion.MoveDirection.sqrMagnitude < m_CharacterLocomotion.ColliderSpacingCubed) {
                    m_ClimbState = ClimbState.Climb;
                    SetAbilityIntDataParameter(AbilityIntData);
                }
            }
...
#if UNITY_EDITOR
            Debug.DrawRay(m_Transform.TransformPoint(offset), (m_CastDistance + Mathf.Abs(offset.z)) * 2 * m_Transform.forward, Color.red);
#endif
