...
namespace Opsive.UltimateCharacterController.AddOns.Climbing
{
    ...
    public class FreeClimb : TraversalClimb
    {
        ...
        [SerializeField] protected float m_TopEdgeOffset = 1.63f;
        ...
        [SerializeField] protected Vector3 m_FootIKOffset = new Vector3(0, 0, -0.17f);
        ...
        public override bool CanStartAbility()
        {
            ...
            if (m_CharacterLocomotion.Grounded) {
                ...
                var position = m_Transform.TransformPoint(0, -0.1f, (m_CastDistance + Mathf.Abs(m_CastOffset.z)) * 2);
                if (Physics.Raycast(position, -m_Transform.forward, out m_RaycastResult, (m_CastDistance + Mathf.Abs(m_CastOffset.z)) * 2, m_DetectLayers, QueryTriggerInteraction.Ignore)) {
                    ...
                    if (Physics.Raycast(MathUtility.TransformPoint(m_RaycastResult.point, Quaternion.LookRotation(m_RaycastResult.normal, m_CharacterLocomotion.Up), new Vector3(0, m_CharacterLocomotion.Radius, 0.1f)),
                ...
                if (!Physics.SphereCast(m_Transform.TransformPoint(m_CastOffset), m_CharacterLocomotion.Radius, m_Transform.forward, out m_RaycastResult, (m_CastDistance + Mathf.Abs(m_CastOffset.z)) * 2, m_DetectLayers, QueryTriggerInteraction.Ignore) || 
                ...
                if (!Physics.Raycast(m_Transform.position + m_CharacterLocomotion.Up * m_CharacterLocomotion.Height, m_Transform.forward, out m_RaycastResult, (m_CastDistance + Mathf.Abs(m_CastOffset.z)) * 2, m_DetectLayers, QueryTriggerInteraction.Ignore) ||
                    (m_RaycastResult.transform != null && !ValidateObject(m_RaycastResult.collider.gameObject, m_RaycastResult))) {
                    return false;
                }
        ...
        public override MoveTowardsLocation[] GetMoveTowardsLocations()
        {
            ...
            if (m_ClimbState == ClimbState.TopMount) {
                ...
                if (m_BottomMoveTowardsLocation == null) {
                    m_BottomMoveTowardsLocation = new MoveTowardsLocation[1];
                    m_BottomMoveTowardsLocation[0] = new GameObject(m_GameObject.name + " Free Climb Bottom Mount Location").AddComponent<MoveTowardsLocation>();
                    m_BottomMoveTowardsLocation[0].Offset = Vector3.zero;
                    m_BottomMoveTowardsLocation[0].YawOffset = 0;
                }

                var rotation = Quaternion.LookRotation(m_RaycastResult.normal, m_CharacterLocomotion.Up);
                var localPosition = MathUtility.InverseTransformPoint(m_RaycastResult.point, rotation, m_Transform.position);
        ...
        public override void Update()
        {
            ...
            var raycastNormal = Vector3.ProjectOnPlane(m_Transform.forward, m_CharacterLocomotion.Up).normalized;
            ...
            if (!Physics.SphereCast(m_Transform.TransformPoint(m_CastOffset), m_CharacterLocomotion.Radius, raycastNormal, out m_RaycastHit, 
            ...
            if (m_CharacterLocomotion.RawInputVector.y > 0) {
                ...
                if (m_TopStop || Physics.Raycast(m_Transform.position, m_CharacterLocomotion.Up, out m_RaycastHit, m_CharacterLocomotion.Height, 
                ...
                if ((m_AllowedMovements & AllowedMovement.TopDismount) != 0) {
                    ...
                    var rayPosition = m_Transform.TransformPoint(0, m_TopDismountOffset + m_CharacterLocomotion.Radius + m_CharacterLocomotion.ColliderSpacing, -Mathf.Abs(m_CastOffset.z));
#if UNITY_EDITOR
                    if (m_DrawDebugLines) {
                        Debug.DrawRay(rayPosition, raycastNormal, Color.green);
                    }
#endif
                    if (!Physics.SphereCast(rayPosition, m_CharacterLocomotion.Radius, raycastNormal, out m_RaycastHit, 
                ...
                if ((m_AllowedMovements & AllowedMovement.BottomDismount) != 0 &&
                            m_CharacterLocomotion.SingleCast(-m_CharacterLocomotion.Up * m_BottomDismountOffset, m_Transform.TransformDirection(0, 0, m_CastOffset.z), m_CharacterLayerManager.SolidObjectLayers, ref m_RaycastHit)) {
            ...
            if (m_CharacterLocomotion.RawInputVector.sqrMagnitude > 0) {
                m_TopStop = false;
                var rayPosition = m_Transform.TransformPoint(m_CharacterLocomotion.RawInputVector.x * ((m_SpeedChange != null && m_SpeedChange.IsActive) ? m_LeapEdgeOffset.x : m_HorizontalEdgeOffset),
                                                                (m_CharacterLocomotion.RawInputVector.y > 0 ?
                                                                    ((m_SpeedChange != null && m_SpeedChange.IsActive) ? m_LeapEdgeOffset.y : m_TopEdgeOffset) : m_BottomEdgeOffset), 0);
#if UNITY_EDITOR
                if (m_DrawDebugLines) {
                ...
                if (Mathf.Abs(m_CharacterLocomotion.RawInputVector.x) > 0 && m_CharacterLocomotion.SingleCast(rayDirection * ((m_SpeedChange != null && m_SpeedChange.IsActive) ? m_LeapEdgeOffset.x : m_HorizontalEdgeOffset),
                                                        m_Transform.TransformDirection(0, 0, m_CastOffset.z * 2), m_CharacterLayerManager.SolidObjectLayers, ref m_RaycastHit)) {
#if ULTIMATE_CHARACTER_CONTROLLER_AGILITY
                    // The Hang ability may be able to start.
                    if (m_Hang != null && m_Hang.TryStartFromClimb(this)) {
                        return;
                ...
                if (!m_CharacterLocomotion.SingleCast((m_CastDistance + Mathf.Abs(m_CastOffset.z)) * 2 * raycastNormal, rayPosition - m_Transform.position,
                    m_CharacterLayerManager.SolidObjectLayers, ref m_RaycastHit) || m_RaycastHit.transform != m_RaycastResult.transform) {
                    ...
                    if ((m_SpeedChange == null || !m_SpeedChange.IsActive) && (m_AllowedMovements & AllowedMovement.OuterCornerTurn) != 0 && Mathf.Abs(m_CharacterLocomotion.RawInputVector.x) > 0) {
                        rayPosition = MathUtility.TransformPoint(rayPosition, rayRotation, Vector3.forward * m_CastDistance);
        ...
        public override void ApplyPosition()
        {
            ...
            var offset = m_Transform.InverseTransformPoint(m_RaycastResult.point).z - m_ClimbOffset;
            var localMoveDirection = m_Transform.InverseTransformDirection(m_CharacterLocomotion.MoveDirection);
            localMoveDirection.z = Mathf.MoveTowards(localMoveDirection.z, offset, m_MoveTowardsSpeed * m_CharacterLocomotion.TimeScale);
        ...
        {
            if (m_ClimbState != ClimbState.Climb || (m_SpeedChange != null && m_SpeedChange.IsActive)) {
                return;
            }
            ...
            var raycastNormal = Vector3.ProjectOnPlane(m_RaycastResult.normal, m_CharacterLocomotion.Up).normalized;
            if (!Physics.Raycast(ikPosition + raycastNormal * (Mathf.Abs(m_CastOffset.z) + 0.1f), -raycastNormal, out m_RaycastHit, (m_CastDistance + Mathf.Abs(m_CastOffset.z)) * 2, m_DetectLayers, QueryTriggerInteraction.Ignore)) {
        ...
        public override bool ShouldBlockAbilityStart(Ability startingAbility)
        {
            if (startingAbility is SpeedChange) {
                ...
                var rayPosition = m_Transform.TransformPoint(Mathf.Sign(m_CharacterLocomotion.RawInputVector.x) * m_LeapEdgeOffset.x, m_CharacterLocomotion.RawInputVector.y > 0 ? m_LeapEdgeOffset.y : 0, 0);
                var raycastNormal = Vector3.ProjectOnPlane(-m_Transform.forward, m_CharacterLocomotion.Up).normalized;
                ...
                if (m_CharacterLocomotion.RawInputVector.x != 0) {
                    ...
                    if (m_CharacterLocomotion.SingleCast(rayDirection * m_LeapEdgeOffset.x, Vector3.zero, m_CharacterLayerManager.SolidObjectLayers, ref m_RaycastHit)) {
                        return true;
                    }
                }
            }
...
#if UNITY_EDITOR
            if (m_DrawDebugLines) {
                Debug.DrawRay(m_Transform.TransformPoint(offset), (m_CastDistance + Mathf.Abs(offset.z)) * 2 * m_Transform.forward, Color.green);
            }
#endif
            if (!Physics.Raycast(m_Transform.TransformPoint(offset), m_Transform.forward, out m_RaycastResult, (m_CastDistance + Mathf.Abs(offset.z)) * 2, m_DetectLayers, QueryTriggerInteraction.Ignore) ||
